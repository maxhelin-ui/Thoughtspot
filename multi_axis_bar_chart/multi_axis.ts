...
// (button-row wrap decisions in adjustButtonContainer, label auto-rotation)
// go stale until something forces a re-render — which used to require a full
// page reload. Watch the chart container and re-run the full render,
// debounced, and only when the size really changed and the first data render
// has happened.
...
(async () => {
    ...
    console.error('multi_axis_bar_chart: failed to initialise chart context:', error);
});
