...
// Bundle Highcharts instead of loading it from CDN <script> tags in
// index.html. A slow or failed CDN fetch left `Highcharts` undefined when
// the first render fired, so the tile failed to load (blank until reload);
// CDN scripts are also blocked under a default-src 'self' CSP. Bundling from
// npm (same approach as custom_bar_chart / violin_plot) removes the network
// dependency entirely. Typed `any` to match the previous
// `declare const Highcharts: any` global.
...
interface VisualProps {
    ...
    // When the active Y is percent-formatted, pin the Y-axis min at 0 so
    // narrow ranges (e.g. 80–85%) aren't exaggerated by Highcharts auto-scaling.
    percentYAxisStartAtZero?: boolean;
    // Per-measure override: force-treat a bound y measure as a percent (or not),
    // regardless of column name / format. Keyed by `measureAsPercent_<colId>`.
    // Matches the equivalent prop in multi_axis_bar_chart.
    [key: string]: any;
}
...
function render(ctx: CustomChartContext) {
    ...
    const percentYAxisStartAtZero = visualProps.percentYAxisStartAtZero ?? false;
    ...
    //
    // For measures only, an explicit per-measure override
    // (`measureAsPercent_<colId>`) wins — matching the multi-axis-bar pattern,
    // so users can force a column to be treated (or not treated) as a percent
    // regardless of its name/format.
    const measurePercentOverride = activeY.kind === 'measure'
        ? visualProps[`measureAsPercent_${activeY.column.id}`]
        : undefined;
    const yIsPercent = typeof measurePercentOverride === 'boolean'
        ? measurePercentOverride
        : activeY.kind === 'measure'
...
// reflows its own plot when the tile resizes, but the pixel-measured pieces
// (alignButtonAreasToPlot paddings, button/legend wrap decisions) go stale
// until something forces a re-render — which used to require a full page
// reload. Watch the chart container and re-run the full render, debounced,
// and only when the size really changed and the first data render has
// happened.
...
function measureChartContainer(): { width: number; height: number } {
    // Watch the whole grid layout, not just the #chart cell: button-area
    // padding changes resize the #chart cell during render, which would
...
(async () => {
    const ctx = await getChartContext({
        ...
        visualPropEditorDefinition: (chartModel: ChartModel) => {
            ...
            yCols.forEach((col, i) => {
                ...
                // Per-measure "Format as %" override. Default mirrors the
                // auto-detection heuristic on the active column name so the
                // checkbox shows the current behaviour by default; toggling
                // it explicitly overrides the heuristic.
                measureControls.push({
                    key:          `measureAsPercent_${col.id}`,
                    type:         'checkbox' as const,
                    defaultValue: detectPercentByName(col.name, (col as any)?.format?.pattern),
    ...
    console.error('multi_y_axis_line: failed to initialise chart context:', error);
});
